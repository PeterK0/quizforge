...
export default function ExamResultsPage() {
  ...
  useEffect(() => {
    if (!state || !state.questions || !state.exam) {
      return;
    }
  ...
  const exportResults = () => {
    ...
    let content = `Exam Results - ${exam.name}\n`;
    content += `Date: ${date}\n`;
    content += `Score: ${totalScore}/${maxScore} (${percentage}%)\n`;
    content += `Status: ${passed ? 'PASSED' : 'FAILED'}\n`;
    content += `Passing Score: ${exam.passingScorePercent}%\n\n`;
    ...
    a.href = url;
    a.download = `exam-results-${exam.name.replace(/\s+/g, '-')}-${Date.now()}.txt`;
  ...
  return (
    ...
    >
      ...
      <div className="bg-bg-secondary border border-border rounded-lg p-6 mb-6">
        ...
        {!passed && (
          <div className="mt-4 p-4 bg-accent-yellow/20 border border-accent-yellow rounded-lg text-accent-yellow text-sm">
            Passing score: {exam.passingScorePercent}% - You need to score at least{' '}
            {exam.passingScorePercent}% to pass this exam.
          </div>
        )}
      </div>

      {/* Question Review */}
      {exam.showAnswersAfter !== 'NEVER' && (
      ...
      <div className="mt-8 flex gap-3 justify-center">
        <Button
          variant="secondary"
          onClick={() => navigate(`/subjects/${subjectId}/exams/${examId}/exams`)}
        >
          <Home size={20} />
          Back to Quizzes
        </Button>
        <Button
          onClick={() =>
            navigate(
              `/subjects/${subjectId}/exams/${examId}/exams/${exam.id}/take`
            )
          }
        >
          Retake Quiz
